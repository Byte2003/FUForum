...
export class UsersComponent implements OnInit {
  ...
  showEditModal() {
    ...
    const initialState = {
      entityId: this.selectedItems[0].id.toString()
    };
    this.bsModalRef = this.modalService.show(UsersDetailComponent,
      {
  ...
  loadUserRoles() {
    ...
    if (this.selectedItems != null && this.selectedItems.length > 0) {
      const userId: any = this.selectedItems[0].id;
  ...
  deleteRolesConfirm(roleNames: string[]) {
    this.blockedPanelRole = true;
    this.usersService.removeRolesFromUser(this.selectedItems[0].id.toString(), roleNames).subscribe(() => {
      this.loadUserRoles();
      this.selectedRoleItems = [];
      this.notificationService.showSuccess(MessageConstants.DELETED_OK_MSG);
      setTimeout(() => { this.blockedPanelRole = false; }, 1000);
  ...
  addUserRole() {
    ...
    const initialState = {
      ...
      userId: this.selectedItems[0].id.toString()
    };
